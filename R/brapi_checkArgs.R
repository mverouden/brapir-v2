...
brapi_checkArgs <- function(usedArgs, reqArgs) {
  ...
  if (length(usedArgs) > 0) {
    ...
    for (i in names(usedArgs)) {
      ...
      if (i %in% c("decimalPlaces", "end", "listSize", "numberOfSamples","page", "pageSize", "plateIndex", "start", "year") || grepl(pattern = "(max)|(Max)|(min)|(Min)|(imageFileSize)|(imageHeight)|(imageWidth)", x = i)) {
