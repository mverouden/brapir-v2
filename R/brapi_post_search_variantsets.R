...
#' @param callSetDbIds vector of type character; required: FALSE; The `CallSets`
#'    to search as specified by the unique database identifier(s); default:
...
#'
#' @details Submit a search request for `VariantSets`. Function will return
...
#'
#' @references \href{https://app.swaggerhub.com/apis/PlantBreedingAPI/BrAPI-Genotyping/2.0#/Variant%20Sets/post_search_variantsets }{BrAPI SwaggerHub}
#'
...
#'
#' @export
brapi_post_search_variantsets <- function(con = NULL,
                                          callSetDbIds = '',
                                          studyDbIds = '',
                                          studyNames = '',
                                          variantDbIds = '',
  ...
  try({
    ...
    if (httr::status_code(resp) == 200) {
      ...
      stop(paste0("The POST /search/variantsets call resulted in Server status, ", httr::http_status(resp)[["message"]]))
    }
  ...
  ## Set class of output
  class(out) <- c(class(out), "brapi_post_search_variantsets")
