...
#' @param searchDateRangeEnd character; required: FALSE; The start of the overlapping search date range. `searchDateRangeStart` must be before `searchDateRangeEnd`.
#' Return a Trial entity if any of the following cases are true
#' - `searchDateRangeStart` is before `trial.endDate` AND `searchDateRangeEnd` is null
#' - `searchDateRangeStart` is before `trial.endDate` AND `searchDateRangeEnd` is after `trial.startDate`
#' - `searchDateRangeEnd` is after `trial.startDate` AND `searchDateRangeStart` is null
...
#' @param sortBy character; required: FALSE; Sort order. Name of the field to sort by.
...
#'
#' @details Retrieve a filtered list of breeding Trials. A Trial is a collection of Studies
#'
#' @return data.frame
#'
...
#'
#' @export
brapi_get_trials <- function(con = NULL, active = NA, commonCropName = '', contactDbId = '', programDbId = '', locationDbId = '', searchDateRangeStart = '', searchDateRangeEnd = '', studyDbId = '', trialDbId = '', trialName = '', trialPUI = '', sortBy = '', sortOrder = '', externalReferenceID = '', externalReferenceSource = '', page = 0, pageSize = 1000) {
